package com.gachon.myapplication123123;
...
public class HandsResultGlRenderer implements ResultGlRenderer<HandsResult> {
    ...
    public void renderResult(HandsResult result, float[] projectionMatrix) {
        ...
        for (int i = 0; i < numHands; ++i) {
            ...
            for (int ind = 0; ind < result.multiHandLandmarks().get(i).getLandmarkList().size(); ind++) {
                NormalizedLandmark lm = result.multiHandLandmarks().get(i).getLandmarkList().get(ind);
                Log.d(TAG, "LandMark[" + ind + "] | x : " + lm.getX() + ", y : " + lm.getY() + ", z : " + lm.getZ());
            }
